...
name = st.text_input("이름", value="예준")
gender = st.selectbox("성별", ["여자", "남자"])

col1, col2, col3 = st.columns(3)

with col1:
    age = st.number_input("나이", min_value=1, step=1, value=20)

with col2:
    height = st.number_input("키(cm)", min_value=1.0, value=165.0)

with col3:
    weight = st.number_input("몸무게(kg)", min_value=1.0, value=55.0)
...
if total == 0:
    st.write("먹은 음식을 선택하면 피드백이 제공됩니다.")
elif total > daily_calorie:
    st.error("오늘 권장 칼로리를 초과했어요. 내일은 저녁을 조금 가볍게 먹는 것을 추천해요.")
elif total < daily_calorie - 500:
    st.warning("오늘 섭취량이 너무 적을 수 있어요. 단백질과 탄수화물을 균형 있게 챙겨 먹는 것을 추천해요.")
else:
    st.success("오늘은 목표 칼로리에 잘 맞게 먹었어요!")

if goal == "감량":
    st.write("감량 목표이므로 고단백, 저칼로리 식단을 추천합니다.")
elif goal == "근육증가":
    st.write("근육 증가 목표이므로 단백질 섭취를 충분히 하는 것이 좋아요.")
else:
    st.write("유지 목표이므로 현재 식사 균형을 유지하는 것이 좋아요.")
